...
if(isset($_POST['add_to_cart'])){
    ...
    // stock check
    $product_info_q = mysqli_query($conn, "SELECT stock FROM `products` WHERE id = '$product_id'") or die('query failed');
    $product_info = mysqli_fetch_assoc($product_info_q);
    $stock_available = intval($product_info['stock'] ?? 0);
...
<section class="products" style="padding-top: 0;">
   ...
   <div class="box-container">
      ...
      <form action="" method="POST" class="box">
         ...
         <input type="submit" value="add to cart" name="add_to_cart" class="btn" <?php echo intval($fetch_products['stock'] ?? 0) > 0 ? '' : 'disabled style="opacity:.5;cursor:not-allowed;"'; ?>>
      </form>
      <?php
         }
            }else{
