...
if(isset($_POST['update_quantity'])){
    ...
    $cart_item_query = mysqli_query($conn, "SELECT pid FROM `cart` WHERE id = '$cart_id' AND user_id = '$user_id' LIMIT 1") or die('query failed');
    if(mysqli_num_rows($cart_item_query) > 0){
        $cart_item = mysqli_fetch_assoc($cart_item_query);
        $product_id = (int)($cart_item['pid'] ?? 0);

        if($product_id > 0){
            $product_query = mysqli_query($conn, "SELECT stock FROM `products` WHERE id = '$product_id' LIMIT 1") or die('query failed');
            $product_row = mysqli_fetch_assoc($product_query);
            $available_stock = (int)($product_row['stock'] ?? 0);

            if($available_stock <= 0){
                mysqli_query($conn, "DELETE FROM `cart` WHERE id = '$cart_id' AND user_id = '$user_id'") or die('query failed');
                $message[] = 'This product is out of stock and was removed from your cart.';
            }elseif($cart_quantity > $available_stock){
                mysqli_query($conn, "UPDATE `cart` SET quantity = '$available_stock' WHERE id = '$cart_id' AND user_id = '$user_id'") or die('query failed');
                $message[] = 'Only '.$available_stock.' item(s) available. Quantity adjusted to stock.';
            }else{
